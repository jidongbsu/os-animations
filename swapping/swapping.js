...
//        "color":"#98DDDD"},
//        we change the color to indicate that this page is not present in the physical memory, but is rather on the disk.
        "color":"#FF0000"},
    {"text":"(unused)",
        "color":"#B7B5B5"},
...
// |  No return value the shape is added to the layer by the function                                              |
// -----------------------------------------------------------------------------------------------------------------
function drawPageTableRow(x, y, w, h, vpn, fpn, vb, pb, preb)
{
    ...
    drawRectWithText(x+4*w,y,w,h,preb,"#ffffff","pte:preb"+vpn)
}
...
drawPageTableRow(ptXstart,ptYstart+ptCellheight,ptCellWidth,ptCellheight,0,pageTable[0],"True","False","False")
drawPageTableRow(ptXstart,ptYstart+ptCellheight*2,ptCellWidth,ptCellheight,1,pageTable[1],"False","True","False")
drawPageTableRow(ptXstart,ptYstart+ptCellheight*3,ptCellWidth,ptCellheight,2,pageTable[2],"True","True","False")
drawPageTableRow(ptXstart,ptYstart+ptCellheight*4,ptCellWidth,ptCellheight,3,pageTable[3],"True","True","True")
...
var programBoxWidth=465;
var programBoxHeight=490;
...
function startAnimation(){
    ...
    for (let i = 0; i < 8; i++) {
        ...
        stage.find('#rec:pte:preb' + virtualPageNumber).fill("#ffffff");
    }
...
function translateAddress() {
    if(animationStep===0){
        ...
        if(tlb.includes(virtualPageNumber)) {
            ...
            stage.find('#rec:pte:preb' + virtualPageNumber).fill(selectedColor);
        ...
        if(tlb.includes(virtualPageNumber)) {
            ...
            stage.find('#txt:prog:23').fontStyle('bold');
        ...
        if(virtualPageNumber===0){
            ...
            stage.find('#txt:prog:22').fontStyle('bold');
            animationStep++;
        }else if(virtualPageNumber === 3){ //page 3
            stage.find('#txt:prog:18').fontStyle('normal');
        ...
        if(virtualPageNumber===2){
            stage.find('#txt:prog:22').fontStyle('normal');
            stage.find('#txt:prog:23').fontStyle('bold');
            stage.find('#txt:result').text("Error: PAGE FAULT").fill("red");
            animationStep=0;
        }else{
